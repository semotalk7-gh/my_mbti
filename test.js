...
const qEl=document.getElementById('qtext'),aEl=document.getElementById('optA'),bEl=document.getElementById('optB'),stepEl=document.getElementById('stepText'),axisEl=document.getElementById('axisText'),progEl=document.getElementById('prog');
...
function render(){const it=QUESTIONS[state.idx]; if(!it){return finish();}
 ...
 aEl.onclick=()=>choose(it.a[0].v); bEl.onclick=()=>choose(it.a[1].v);
...
function choose(v){state.answers.push(v); state.score[v]=(state.score[v]||0)+1; state.idx++; if(state.idx>=QUESTIONS.length){finish();} else render();}
